...
# так как объекты типа int не имеют длину. Мы не можем применить эту ф-ю к целым числам
# Или при конкотенации разных типов объектов print(my_list + 'hello') - тоже плучим ошибку.
...
# В дальнейшем, при обработке этого начения None в программе могут возникнуть ошибки, так как мы ожидаем строку.
...
# Когда мы создаем свои ф-и, желательно сздавать к ним документации, где давать инструкции, предупреждающие
# возможные ошибки
...
user_dictionary = {'firsrt_name': 'Jack', 'last_name': 'White', 'age': 24}

# Выводим данные из словаря
# print(user_dictionary['firsrt_name'])
...
# Но, если мы получаем это значение при помощи ф-и get(), котора существует для словаря
# print(user_dictionary.get('firsrt_name'))  # Jack
# print(user_dictionary.get('ame'))  # None
# При использовании ф-и get() в случае ошибки получаем None, То есть, мы не получаем ошибку
# В этом метода ошибки обрабатываются.
...
print(get_dict_value(user_dictionary, 'firsrt_name'))  # получаем Jack
