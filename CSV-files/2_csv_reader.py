...
# Имеем готовый файл cars.csv, в котором содержатся данные о машиннах в табличном формате

with open('cars.csv') as file:
    csv_reader = csv.reader(file)  # в переменную csv_reader мы получаем итератор.
    # теперь мы можем перебирать каждую строку из переданного файла и они будут выводиться в форме списка.
    ...
    for car in csv_reader:
        ...
        # Чтобы пропустить первую строку, мы можем, перед тем, как распечатывать, вызвать ф-ю next()
