...
with open('car_data.csv', 'w') as f:  # создаем файл car_data.csv в режиме записи 'w'
    ...
    # построчно данные из файла car_data
    # Будут записаны данные в файл построчно, как они даны в строке.
    # Запускаем код
...
with open('car_data.csv', 'r') as f:  # Считывание происходит в режиме 'r'
    reader = csv.reader(f, delimiter='&')  # знак разделителя '&' говорит, что работатет разделитель по умолчанию
    for row in reader:
        print(row)
        # ['Brand', 'Price', 'Year']
        # []
...
# Весть верхний код можно записать более коротким способом -
