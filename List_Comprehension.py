...
letter_list = [letter for letter in greeting]  # Читается как: для каждой буквы letter из строки greeting
# помещаем букву letter в этот список
print(letter_list)
...
# этот элемент в список. Например, возвести каждый элемент в квадрат и затем поместить в список
...
# Для Списков List Comprehension. Вносим Список в Словарь, где ключ - цифры из скиска, значения - результат вычислений над ними.
...
# Нам надо сгенерировать из исходного списка другой список, в который попадут имена, начинающиеся с буквы 'J'
# Сделать для этого генератор списка

new_names = [n for n in names if n.startswith('J')]
# startswith() - это встроенный метод строк, который проверяет начинается ли строка с конкретного элемента
# Возвращает True или False. Также существует его аналог для последнего знаrf endwith()

# Эта же запись в традиционном виде:
new_names_2 = []
for n in names:
    if n.startswith('J'):
        new_names.append(n)
