...
# Для описания объектов при помощи языка программирования, используются такие понятия, как классы и объекты.
...
# свой собственный класс с атрибутами и методами, которые нужны нам для реализации кокой-то функциональности
...
class MyFirstClass:
    # Сохраняется отступ слева в 4 пробела или в 1 таб
...
# Даем ему название, через знак '=' говорим, что это объект класса Car
# и дальше описываем его характеристики.
...
bmw_car = Car('BMW 7s', 'Black', 2018, False)  # Присваиваем значения его атрибутам
# (сами атрибуты указывать не обязательно)
...
# Так как это атрибут класса, то мы не можем обращаться к нему через объект, а только через сам класс
