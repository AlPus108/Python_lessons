...
# Здесь мы создадим свой небольшой модуль с арифметическими операциями и ф-ей вычисления чисел Фибоначчи.
...
# Создадим еще одну дирректорию main_exe.py и импорируем туда этот модуль my_math_module.py  1--> переходим в main_exe.py
...
# 4--> Проверяем. Исполняем этот модуль
# Проверка прошла успешно
# Если мы распечатаем значение переменной __name__
print(__name__)
# Она будет иметь значение __main__  5--> main
