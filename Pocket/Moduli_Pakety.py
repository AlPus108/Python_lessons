...
# что на каждом этапе разработки мы должны прикладвыать максимум усилий в то, сложность нашего проекта не возрастала.
...
# Модуль и Пакет, это есть инструменты декомпозициии програмного кода. Модуль, это маленький элемент декомпозиции,
...
# Он похож на передачу некой коробки
...
# Он позволяет добавлять и удалять папки, смотроеть адресе файлов и другие функции файлового менеджера или проводника.
import os  # импорт всего модуля

# После импорта модуля, мы можем вызывать любую ф-ю из данного модуля путем следующего кода:

print(os.getcwd())  # ф-я для выявления текущей дирректории/папки, в которой ведется работа.
# C:\Users\Lenovo 330s\PycharmProjects\Python_lessons

# Импорт конкретной ф-и
from os import mkdir

# теперь мы можем создавать использовать данную ф-и и создавать дирректории/папки без приамбулы os.
# mkdir('test')
# Повторный вызов этого кода приедет к ошибке, так как папка уже создана.
# Поэтому закомментировать после создания дирректории
...
# это ф-я walk(). На вход для нее мы должны подать дирректорию, из которой мы хотим получить информацию.
...
# Чтобы импортировать пакет, он должне находится в дирректории, которая должна входить в специальный список,
...
# Можно создать свой собственный пакет и залить его на pocket index - это портал, откуда можно захрузить
