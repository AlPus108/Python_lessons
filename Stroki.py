...
# str[:5] = 'HELLO' - вывод строки до последнего (пятого по индексу) элемента (также, получаем всю строку)
...
print(week_days)
# Но, если значений очень много, их положение предтся вычитывать. Поэттому можно присвоить им ключи
...
# Явно, что нам не нужно такое большое количество дробных знаков. Нам хватит и тех. Как убрать остальные?
...
# например в виде таблицы, чтобы выровнять цифры на одном уровне.
