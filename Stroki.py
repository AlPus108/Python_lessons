...
# str[0:] = 'HELLO' - вывод строки с первого элемента (также, получаем всю строку)
# str[:5] = 'HELLO' - вывод строки до последнего элемента (также, получаем всю строку)
# str[:3] = 'HEL' - вывод строки до 3-го элемента (сам 3-й элемент не включается)
...
# Все специальные символы в Питоне экранируются с помощью обратного слэша "\", в том числе и обратный слэш \\

# ------------------ ОБРАЩЕНИЕ К СИМВОЛАМ ------------------
# Вывод одной буквы
# print(temp_str[0])
#
# Вывод всй строки посимвольно
...
#  Вообще не рекомендуесят пользоваться плюсом. Нужно пользоваться фарматированием.
...
# Второй более удобный способ форматирование строк - через декоратор f (появился в версии 3 Питона)
...
# Часто стребуется для парсинга строк, разбиение строки на подстроки. Для этого создали сетод split()
print(temp_str.rsplit())
...
# for i in s[1:]:  # листаем строку в цикле
