...
# Если мин нет, мы должны посчитать количество мин в соседних клетках

# Расчитываем размер таблицы и количетсво мин
stroki, stolbci, miny = (int(i) for i in input().split())  # получаем входящие значения
# Инициализируем двумерный массив - генерируем n строк и в каждой строке герерируем m нулевых значений
a = [[0 for j in range(stolbci)] for i in range(stroki)]
...
for i in range(miny):  #
    ...
    # мы сразу в преобразованном числе вычитаем 1. Полученые индексы row и col начинаюстя с нуля.
        ...
        if a[i][j] == 0:  # если в ячейке записан 0, то есть нет мины
            for di in range(-1, 2):  # перебираем все клетки со смещение верх-вниз-влево-вправо по коорд i
                for dj in range(-1, 2):  # аналогично по координате j
                    ...
                    if 0 <= ai < stroki and 0 <= aj < stolbci < stolbci and a[ai][aj] == -1:
...
for i in range(stroki): # Для этого перебираем все строки таблицы
    for j in range(stolbci): # и в строке перебираем все элементы
