...
# импортируем модуль Moduli_Pakety.py
...
# 6 --> А если мы запустим этот модуль main, здесь также выведется значение переменной __main__, которая находится
# в модуле my_math_module, но теперь у нее значение будет my_math_module
# То есть эта переменная связывает два этих модуля и в каждом модуле она показывает связанный с нии модуль.
...
# иначе, этот код будет исполняться каждый раз, когда вы импортируете модуль.
...
# math_op.py
# math_func
# В math_op копируем математические ф-и из вкладки my_math_module
# В math_func копируем ф-ю вычисления ряда чисел Фибоначчи
...
from Pocket import my_add

# И теперь мы вызываем ф-ю my_add без личних пиставок
print(my_add(8,6))

# но ф-ю my_div() сейчас мы так вызвать не можем, потмоу что на не прописана в __init__.py
...
# Вы можете где-то отдельно хранить свои пакеты и добавоять в sys.path методом .append любую дирректорию,
# откуда вы хотите подтаксивать пакет. sys.path, это обычный лист/список
# sys.path.append('.......')
