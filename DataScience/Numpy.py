...
# Потому что массивы NumPy имеют строгую типизацию. Один только int имеет в NumPy четыре типа:
# int8, int16, int32, int64. Важная справка: пользоваться числами больше размерености машины не имеет смысла.
...
# Чтобы ravel() этого н еделал, в аргументы можно вставить 'F'
...
# Очень важный моменрт - это не равнозначно операции - вырезка из списка.
...
# То есть, в этом случае мы не копируем данные из масива, а воздействуем непосредственно на сам массив.

# Тогда, логичный вопрос: как сделать копию, чтобы остававить изначальный массив без изменения?
...
mas_new = mas[3:8]
print(mas_new)  # [ 3  4 10 10 10]  - привоили указанные диапазон из старого массива новому
...
print(a.shape)
# (6,) -  одна строка с шестью позициями. При реализации шейпа, получаем последовательность тапл с 6 в индексе 0
...
for i in range(np_lst_num.shape[0]):  # идем по первой оси [0] - то есть здесь просто делаем выбор строк
    ...
    for j in range(np_lst_num.shape[1]):  # идем по втрой оси [1] - строки в пределах одной колонки, те по одной позиции
...
# команад print() всегда переводи на новую строку, но им делать не удобно.
...
# [75 81 87 93 99]
# Это все в сумме дас предыдущее значене - 435
...
# Одна оговорка - при выполнении этих операций, длина у массивов должна быть одинаковая.
...
# В случае деления числа на 0 оплучаем бесконечность infinity (inf)
# При делении этого массива самого на себя полу чаем то же предупреждение
...
a ** 2  # все элементы массива возвможим во второу степерь
...
# -------------- Ф-и быстрого задания numpy-массивов ---------------------
...
# Поэтому предусмотрены сопсобы создания уже заполненных массивов.
...
d_ = np.linspace(0, 5, 5)  # старт, стоп и количество равномерно распределенных элементов
...
# --------------------------------------- eye() -------------------------------
# Эта ф-я создает Identity matrix / Айдентити матрицу
...
# Это одномерныйы массив. Если хотим получсить двумерный - в скобках помещаем два числа
