...
lst_1 = [1, 4, 8]
lst_2 = ['Один', 'Четыре', 'Восемь']
lst_1.extend(lst_2)
print('Добавление одного списка в другой с помощь extend() ', lst_1)

# ---------------------------- pop() --------------------------------
# Удаляет элемент ПО ИНДЕКСУ или последний, если индекс не указан.
...
# Если элемента не существует, будет выдана ошибка ValueError
...
# -----------------------------sort() --------------------
# Сортирует список на основе ф-и.
...
# ------------------------ index() ----------------------------------
# list.index(x,[start[,end]]) Возвращает положение первого элемента со значением х,
# (при этом поиск ведется от start до end) - необязательные элементы
l = [1, 2, 3]
print('Поиск индекса с помощью ф-и index() ', l.index(2))
...
# -----------------------------clear() ----------------------------
# Удаляет все элементы в списке (очищает список)
# list.clear()


# ---------------------------- copy() ----------------------------
# list.copy() Возвращает копию значений списка
...
int_list = [3, 8, 5, 2, 4, 1, 6, 9, 7,
            0]  # переинициализируем список для следующего примера, так как в последнем действии
...
print(students[:2])  # выводим два первых элемента списка от начала. Элемент с индексом 2 не включен в вывод
# ----------------------------------------------------
...
# a = [int(i) for i in input("Введите строку из чисел с пробелами ").split()]
# input() читает строку
