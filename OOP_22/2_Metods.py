...
class Car:
    ...
    # ------------------------------- МЕТОДЫ ОБЪЕКТОВ ----------------------------------------
    # Методы, это функции, которые описываются или определяются внутри класса.
    ...
    # Этим объекты очень удобны. То есть, они похожи на объекты реального мира.
    ...
    # воздействовать на эти свойства. И не обязательно на свойства объекта, мы можем производить действия любого рода.
    ...
    # Итак, сейчас у нас есть метод drive(), который не имеет никаких параметров,
    # мы не передаем ему при вызове параметры.
...
# Ключевое отличие того, как мы обращаемся к методу и к атрибуту объекта состоит в том,
# что после метода, как при вызове ф-и, мы указываем круглые скобки,
# а при обращении к атрибуту, этих скобок в конце нет.
# Это потому, что метод, это ф-ия и он выполняет какие-то действия.
# Атрибут не выполняет никаких действий и его можно разценивать как переменную объекта,
# которая содержит какое-то значение.

# Если мы выведем на печать метод drive (без скобок)
print(opel_car.drive_2)
...
opel_car.drive_2(' London')  # передаем информацию о городе для параметра city. Он обязателен, та как зашит в метод.
# Opel Tigra is driving to London

# Создаем новый объект
mazda_car = Car('Mazda CX7', 'black', 2018, False)
# Выведем связанный с ним метод drive()
mazda_car.drive_2(' Paris')  # Mazda CX7 is driving to Paris
...
class Circle:
    ...
    # Создаем главный метод (конструктор) класса
    def __init__(self, radius=1):  # добавляем параметр radius со значением по умолчанию - основной параметр круга
        # Если мы не укажем явно какое-то значение, то подставится дефолтное значение 1
        self.radius = radius  # инициализируем атрибут radius

        # Также, мы можем инициализировать какой-то атрибут объекта путем вычисления некоего выражения.
        # То есть, не создавать следующий параметр в скобках метода, а прописать его вычисление при инициализации
        self.circumference = 2 * Circle.pi * self.radius  # параметр circumference создаем при его инициализации
        # и тогда не надо создавать отдельный метод

    # здесь к Пи обращаемся через указание имени класса,
...
print(circle_1.circumference_2())  # 6.28 - длина окружности при радиусе 1

circle_2 = Circle(3)  # теперь передаем значение радиуса нашего круга
# Вызываем метод и выводим его на печать
print(circle_2.get_area())  # 28.26 - площадь круга при радиусе 3
print(circle_2.circumference_2())  # 18.84 - длина окружности при радиусе 1
...
print(circle_3.circumference_2())  # 31.40 - длина окружности при радиусе 5
