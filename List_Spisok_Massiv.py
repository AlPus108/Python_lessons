...
# Результат конкатинации можно в новый список
...
# Вывод:  [1.2, 123, 'Volvo', [1, 2, 3], 'V', 'o', 'l', 'v', 'o']
# Таким образом, через указани индекса можно удалить элемент на любой позиции.

# Методо pop() после своей работы также возвращает удаленный элемент на печать
...
# map - ф-я, которая применяется к каждому элементу списка
# Конструкция map:
...
print(min(students))
# Вывод списка в противоположном порядке. Метод reverse() меняет сам список
students.reverse()
...
# ОСОБЕННОСТИ СПИСКОВ
# То обстоятельство, что списки являются объетками, создает определенные особенности, которые надо учитывать
...
# a = [int(i) for i in input("Введите строку из числел ").split()]
# input() читает строку
...
# и перепрыгиваю "вперёд" на столько элементов, сколько он встречается.
...
# a[1]
# это будет списко их трех элементов
# [4,5,6]
