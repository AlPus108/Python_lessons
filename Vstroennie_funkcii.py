...
# Но, это же мы можем сделать по другому, более коротким путем с помощью ф-и enumerate().
# Такой вариант кода намного корече и понятнее. Нам не надо создавать вспомог.переменную и постоянно ее приращивать.

my_string_3 = 'knwehnvslcj'
for item in enumerate(my_string_3):
    print(item)
# Вывод:
# (0, 'k')
# (1, 'n')
...
my_string_4 = 'knwehnvslcj'
for index, letter in enumerate(my_string_4):  # Первое значение распаковываем в index, второе в letter
...
# Для того, чтобы получить доступ к функциям библиотеки, нам нужно сначала импортировать библиотеку
