...
# 1-й - function, 2-й - iter (то есть iterable - последовательность каких-то элементов)
...
# Мар применяет ф-ю len() к каждому элементу данного списка
...
def sum_of_one_num(x):  # эта ф-я возвращает сумму одного аргумента, который складывается с самим собой
    return x + x
...
# Конечно, мы можем использовать для этого цикл for и в цикле вызывать функцию sum_of_one_num для каждого элемента.
...
result = map(sum_of_one_num, num_list)
print(result)
...
for item in map(sum_of_one_num, num_list):
    print('Выводим список через цикл ', item)

# То же самое, но при помощи List Comprehension и получаем результат в виде списка
rez = [item for item in map(sum_of_one_num, num_list)]
print('Выводим список через цикл ', rez)

# Еще один способ через приведение результата к итпу list (без цикла)
print('Выводим список через list ', list(map(sum_of_one_num, num_list)))
...
print('Выводим нечетную последовательность через filter ', list(filter(is_num_odd, num_list)))
# [1, 3, 5, 7] - получаем список элементов, возвращающих True, то есть нечетные числа
# либо распечатывать в цикле for

for num in filter(is_num_odd, num_list):
    print('Выводим нечетную последовательность через цикл и filter ', num)
# 1
# 3
# 5
# 7
...
def ident(x):
    return x  # Получаем переданное число
...
# Ф-я Лямбда может содержать только выражения и не может включать операторы в свое тело
...
# Применим уже рассмотренную нами ранее более сложную ф-ю
...
# Мы ее можем использовать в выражениях сразу же в качестве параметра
...
# Не нужно писать заранее какую-то ф-ю. Если она не большая, можно записать в одну строчку:
squares = map(lambda x: x*x, [0, 1, 2, 3, 4])
# Здесь возмодим каждый элемент списка в квадрат. При этом сразу же формируем новый массив списка squares
print(list(squares))
# [0, 1, 4, 9, 16]

# Перепишем выражение перевода милей в км (рассмотрели ранее), но тепероь уже с помощью Лямбды

km_dist = list(map(lambda x: 1.6*x, mile_dist))
print(type(km_dist), km_dist)  # <class 'list'> [1.6, 2.5600000000000005, 3.6799999999999997]
...
# Выстроим все элементы в обратном порядке
print(list(map(lambda string: string[::-1], string_list)))
# ['ih', 'saw', 'eman', 'ehs']
...
# В Python 2 Reduce была встроенной ф-ей, как Мар, Filter и sort, в Python 3 она перекочевала в модуль functools,
...
print('Суммируем элементы списка ф-ей Reduce ', sum_all)  # 15
