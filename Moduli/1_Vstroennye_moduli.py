...
# По мимо собственных модулей, созданных программистами, которые можно использовать много раз,
# в Пайтоне существуют так еазываемые build-in модули, то есть встроенные модули.
# Например, мы уже использвовали такой build-in модуль random(), с помощью которого мы получали случайное целое число.
# В Пайтоне очень большое количество встроенных модулей, кторые мы здесь и рассмотрим.
...
# В этом списке сожержится очень большое количество методов. Они скопанованы по алфавиту.
...
# Например, ф-я randint находится в модуле random. Можно ипортировать весмь модуль
import random
...
# Чтобы импортировать одну конкретную ф-ю, надо написать следующее:
...
# Также, при импорте ф-и, можнодать ей удобное нам имя, а не то, под которым оно находится в модуле.
