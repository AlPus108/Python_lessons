...
# В других языках программирования (Джага, Си-шарп) нет множественного наследования. В них класс может унаследоваться
...
class Walkable:
    ...
    def greeting(self):
        print(f'Привет! Меня зовут {self.name} и я могу ходить')  # Здесь он говорит, что может ходить.
...
class Flyable:
    ...
    def greeting(self):
        print(f'Привет! Меня зовут {self.name} и я могу летать')  # Здесь он говорит, что может летать.
...
class GameCharacter(Swimmable, Walkable, Flyable):  # унаследованные классы перечисляются через запятую.
    # И теперь наш персонаж будет меть все эти способности. Это и сесть множественное наследование.
    def __init__(self, name):
        # 19
        # делаем распечатку для ф-и init
...
# Сейчас мы его рассмотрели, чтобы понять, что это возмоно делать и как это работает.
