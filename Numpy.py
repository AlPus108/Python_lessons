...
print(a.shape)
# (6,) -  одна строка с шестью позициями. При реализации шейпа, олучаем последовательность тапл с 6 в индексе 0
...
for i in range(np_lst_num.shape[0]):  # идем по первой оси [0] - то есть здесь просто делаем выбор строк
    ...
    for j in range(np_lst_num.shape[1]):  # идем по втрой оси [1] - колонки в пределах одной строки, те по каждой позиции
