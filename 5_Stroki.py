...
# Создадим списко из слов, которые имеют одинаковую длину

len_list = [0 for i in range(max_word_len)]  # в max_word_len сейчас находится слово длиной 14
print(len_list)  # получакм список из двенадцати пустых ячеек

for l in len_words:  # опредеяем длины слов
    # если у нас длина слова = 1, мы должны в нулевой элемент l добавить 1
    len_list[l - 1] += 1 # если у нас попалось слово с длиной = 0, нам надо в нулевой элемент листа добавить 1
    # если попалось слово длиной 5, в 4-й элемент списка добавляем 1. Почему? Потому что список индексируется с 0.

print(len_list)
# [15, 26, 19, 22, 10, 22, 28, 10, 15, 11, 9, 3, 2, 1]  # это список длин всех слов

# Выведем немного получше
for i in range(len(len_list)):  # Пройдемся по всем элементам
    print('Слов длины ', i + 1, ' - ', len_list[i], sep='')
# Получаем списко, сколько слов и какой длины:
# Слов длины 1 - 15
# Слов длины 2 - 26
# Слов длины 3 - 19
# Слов длины 4 - 22
...
# Здесь первое значение - длина слова, второе - количество этих слов.
