# Функции используются в том лучае, если код повторяется многократно.
# Существует много действий, которые приходится повторять несколько раз. Часто приходится искать что-то максимальнгое,
# минимальное, самое длинноое, самое короткое и т.д. Поэтому такой код пищется один раз и заключается в функцию.
...
# Если поставить точку рядом с именем объекта, среда разработки выдаст список ф-й, которые можно к нему применить
...
# причем что угодно: любые объекты, массивы, кортеджи, списки а может и не возвращать.
# Слова return может и не быть. В этом случае ф-я все равно делает возврат,
# но это будет объект типа None - пустой объект
