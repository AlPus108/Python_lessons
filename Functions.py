# Функции используются в том случае, если код повторяется многократно.
# Существует много действий, которые приходится повторять несколько раз. Часто приходится искать что-то максимальное,
# минимальное, самое длинное, самое короткое и т.д. Поэтому такой код пищется один раз и заключается в функцию.
# И не факт, что она может быть записана на языке Python
...
# Кроме этого, в Пайтоне есть и встроенные методы. Метод, это еще одно назавние ф-и.
...
# Если поставить точку рядом с именем объекта, среда разработки выдаст список методов, которые можно к нему применить
my_list.append(4)  # в скобках мы передаем параметр/аргумент, который эта ф-я обраатывает
...
# Ф-я может возвращать что-то через слово return,
# причем что угодно: любые объекты, массивы, кортеджи, списки
# А может и не возвращать. Слова return может и не быть.
# В этом случае ф-я все равно делает возврат, но это будет объект типа None - пустой объект
...
# Ф-я может получать параметры, а может и не получать.

# Ф-я без параметров
def print_greeting():
    print('Запускаем ф-ю ', 'Hello!')  # Запустив эту ф-ю, мы ничего не получим.
# Дело в том, что мы пока только определили ф-ю. Чтобы ее запустить, надо ее вызвать.
# То есть, написать ее имя в строке.
print_greeting()
...
def print_greeting_2():
    ''' для многострочного комментария всегда делается отступ
    Print 'Hello!' text
    :return:  None
    '''
...
# Минимум для описания ф-и:
# param x: описание
# param y: описание
# return: описание
# Входные аргументы и вывод - этот минимум, который нужно всегда указывать в описании ф-и.

print_greeting_2()
# Далее мы можем посмотреть этот docstring в нашей программе. Это делается при помощи встроенной в Пайтоне ф-и help()
help(print_greeting_2)  # Внутри указываем имя ф-и, но без скобок
...
# Если мы попробуем присвоить какой-то переменной результат этой ф-и
...
def sum_of_two_numbers(a=0, b=0):  # Вводим параметры по умолчанию
    '''
...
# Вызываем эту ф-ю через присвоение переменной
...
# Ф-я может принимать и возвращать любые значения, даже другие функции.

def f1(n):
    def f2(m):  # принимает на вход значение m
        return n + m  # складывает m со значением n
    return f2


new_f = f1(100)  # передаем на вход значение n. Это инициализация n
print(new_f(250))  # с вызовом ф-и передаем на вход значение для m

# Посмотрим на тип
print(type(new_f))
# Вывод: <class 'function'>
# Это функция возвращает результат работы другой ф-и.

# Ф-я может ничего не возвращать

def f():
    pass  # заглушка (служебное слово)
# При вызове такая ф-я возвращает None
...
# Здесь все работает правильно, но это можно записать намного короче
# Оператор in возвращает True или False в зависимости от того, содержится ли какой-то элемент в последовательности
# или нет. Поэтому, нам не обязательно использовать оператор if. Можно сразу писать 'hello' in text.
...
def greeting_depends_on_gender(name, gender):
    if gender == 'male':
        ...
        print('Hello! ' + name + 'Ive never seen the creater like you!')
        return gender
...
def my_range(start, stop, step=1):  # здесь указывается диапазон значений (start - stop) - обязательные аргументы
    # step - необязательный аргумент. Это размер шага, который по умолчанию = 1. При вызове ф-и его можно не указывать
    # Когда мы присваивам аргументу ф-и значение по умолчанию, он становится не обязательным, т.к. уже имеет значение.
    # таким образом все аргументы ф-и проинициализированны. Реализация ф-и не зависит от значения step
    res = []  # пустой список, в который накапливаются элементы, которые нужно будет вернуть.
...
# Если при вызове ф-и мы указываем необязательный параметр со значением отличным от значения по умолчанию,
# то его значение меняется.
# Необязательный параметр всегда указывается в конце списка аргементов.
# Вот так делать нельзя: def f(z = 2, x, y)
...
# xs сначала добавляет 0 в список 'а', и затем связывается с новым списком (объектом), то старая связь уже нарушается,
# ее уже нет.
...
# Так как внутри ф-и зашита переменная 'а', интерпретатор, не найдя ее внутри ф-и, будет искать ее снаружи ф-и,
...
# args (arguments) - аргументы типа Тапл
# kwargs (key word arguments) - аргументы по ключевому слову типа Словарь
# Они используются, чтобы получать в качестве параметра ф-и произвольное количество параметров;
# чтобы не использовать большое количество переменных при определении ф-и.
...
# Что делать, если нам надо передать три аргумента?
...
# Существует способ создать ф-ю с произвольным количеством параметров. В эту ф-ю мы можем передавать
...
def ten_percent_of_product_with_args_01(*args):
    ...
    for number in args:  # и дальше запускаем цикл, который проходится по диапазону цифр, переданному через args
        product = product * number  # в number находится очередное значение из диапазона args
        # получаем произведение всех аргументов из args
...
# неограниченное количество элементов разных типов.
...
def func_with_kwargs(**kwargs):  # сюда передаются параметры в форме пар: ключ - значение, поэтому две звезды
    # Параметры могут быть в любом количестве и любого типа.
    print(kwargs)  # распечатаем "голую" ф-ю, чтобы посомтреть что она выводит
    print('Выводим тип kwargs ', type(kwargs))


# Вызываем ф-ю
func_with_kwargs(first=1, second=2, third=3, a='Volovo', b=1.5)
# Вывод: {'first': 1, 'second': 2, 'third': 3, 'a': 'Volovo', 'b': 1.5}  - получаем последовательность типа Словарь
# Мы можем передавать сюда произвольное количество аргументов и с ними произовдить какие-то действия
# Выводим тип kwargs  <class 'dict'>
...
# Во второй строке идет проверка: если в этой последовательности есть ключ 'name', то выводится третья строка,
...
# При передаче параметр в ф-ю с kwargs ключем обязательно должна быть какая-то строка - key, т.к. words - ключевые слова
...
# На вход можно подавать довльно сложные структуры параметров
def func_difficult(x, y = 2, *args, **kwargs):  # обязательные параметры, необязательные, args и kwargs
    print(type(x), x)
    print(type(y), y)
...
# Эту ф-ю можно вызвать с одним обязательным параметром - х
func_difficult(1)
# Вывод:
# <class 'int'> 1
# <class 'int'> 2
...
func_difficult(1,3, (1,2,3), temp_1=12, temp_2=13)
# Передаем первый арумент, изменяем значение второго необязательного, список типа Тапл, Словарь

# Вывод:
# <class 'int'> 1
# <class 'int'> 3
# <class 'tuple'> ((1, 2, 3),)
# <class 'dict'> {'temp_1': 12, 'temp_2': 13}
...
# Попробовал сделать тот же вариант ф-и тольлко с *args
# Пока выдает ошибки

# def modify_list_args(*args):
#     x = []
#     for i in args:
#         if i % 2 != 0:
#             del i
#         else:
#             x += i
#     lst_3 = [int(i / 2) for i in x]  # Возвращает тип float, поэтому переводим в int
#     print(lst_3)
#
#
...
# Пример.
# У нас есть коробка, в которой множество других коробочек, в одной из которых находится ключ.
# Наша задача - найти коробку с ключом.
# У нас есть ф-я "Найти ключ в коробке", которая открывает коробки и проверяет их на предмет ключа.
# Открыли первую коробку - ключа нет,
# Открыли вторую коробку, которая находится в первой коробке - ключа нет.
# И тд
# Для каждой коробки вызывается функция поиска ключа.
# И каждый раз мы погружаемся все глубже и глубже на новый уровень поиска и каждый раз вызывается одна и та же ф-я.
# Это тот случай, когда ф-я вызывает сама себя. Это и называется - рекурсивная ф-я.
# Рекурсивная ф-я, это функция, вызывающая сама себя.
# Ф-я вызывает самя себя, но каждый раз с разными параметрами, потому что каждый раз обращается к новой коробке:
# Сначала большяа коробка, затем коробки которые в ней, затем коробки в этих коробках и тд.
# Другими словами - Рекурсивные ф-и, это ф-и вызывающие сами себя, но каждыйраз с различными параметрами.

# Традиционный пример рекурсивной функции - вычисление факториала.
# Факториал - число, равное произведению чисел от 1 до n.
# f = 1 * 2 * 3 * 4 * 5....
# Как считается факториал через рекурсивные ф-и?
# Можно посчитать в цикле, но можно посчитаь и через рекурсивные ф-и.

# Вызываем ф-ю factorial(n)
# n - количество коробок
# Если n <= 1 (то есть коробок нет) --> factorial = 1 --> Завершение программы
# Если n >= 1 (коробки есть) --> factorial = n*factorial(n-1) <-- ф-я снова вызывает сама себя,
# проверяет n и в случае >= 1, умножает себя на n-1 (нисходящий порядок перебора)
# затем снова вызывает себя и умножает на n-2 и тд до 1 (до последней коробки)
# Когда n <= 1 (то есть коробок нет) --> factorial = 1 --> Завершение программы

# С рекурсивными ф-ями надо быть крейне осторожными. Каждый раз, когда вы вызываете какую-то ф-ю, для нее резервируется
# место в оперативной памяти. Так как рек.ф-я вызывает сама себя, но не завершает предыдущие вызовы, зарезервированное
# место в памяти для нее постоянно накапливается. Если рекурсия будет глубокой, то она может занять всю опер.память.
# Поэтому, как правило, рекурсии пытаются избегать.
# Но, тем не менее бывают очень красивые решения, которые используют рекурсивные ф-и.
...
# Для сравнения то же самое в цикле
# Вычисление Факториала в цикле

factorial = 1
for i in range(1, 11):  # в скобках указываем до скольки хотим сделать факториал. Если до 10, то указываем 1 - 11
    #                   В range обычно мы начинаем с первого значения и заканчиваем на последнем -1 (0, 10-1)
    factorial *= i
print(factorial)  # 3628800


# Ф-я возведения в степень а^b c помощью факториала

def degree(a, b):  # два параметра на вход: а - кого возводим, b - на что возводим, в какую степень (кол-во итераций)
    if b == 0:
        return 1
    else:
        return a * degree(a, b-1)  # Здесь мы каждый раз уменьшаем степень (кол-во итераций).
        # То есть, за один проход этой ф-и мы каждый раз домножаем на а то, что уже возвели:
        # a -> a2 -> a3 и т.д. до а в степени b


# Проверяем
print(degree(2, 10))  # Возведем 2^10. Все программисты должны помнить, что это 1024
# 1024

# Та же процедура возведения в степень с помощью цикла

deg = 1  # кого возводим - параметр а
for i in range(1, 11):  # количество итераций - параметр b = 10+1 = 11
    deg *= 2  # a
print(deg)  # 1024

# Рекурсию редко используют в промышленном коде, но иногда попадаются очень красивые решения,
# которые используют рекурсивные ф-и.
# Но, если можно обойтись без рекурсии, то лучше ее не использовать.